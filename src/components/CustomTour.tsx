import { ArrowLeft, Calendar, MapPin, Check, Car, DollarSign, Route, ExternalLink, Info, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
const destinations = [
  {
    ...
    history: 'Miradouro natural formado há milhões de anos, oferecendo uma das vistas mais impressionantes de Angola.',
    ...
    duration: '2h',
    highlights: ['Vista panorâmica 360°', 'Fotografia profissional', 'Pôr do sol único'],
  },
  {
    id: 2,
    name: 'Serra da Leba',
    ...
    history: 'Obra-prima da engenharia portuguesa construída nos anos 70, tornou-se símbolo nacional de Angola.',
    ...
    duration: '1.5h',
    highlights: ['Miradouro panorâmico', 'Fotografia aérea', 'Arquitectura impressionante'],
  },
  {
    id: 3,
    name: 'Cristo Rei',
    ...
    history: 'Erguido em 1950, o Cristo Rei de Lubango foi inspirado no Cristo Redentor do Rio de Janeiro.',
    ...
    duration: '1h',
    highlights: ['Vista da cidade', 'Símbolo histórico', 'Espaço contemplativo'],
  },
  {
    id: 4,
    ...
    history: 'Quedas de água alimentadas pelos rios da região, criam um oásis natural no planalto.',
    ...
    duration: '2h',
    highlights: ['Banho refrescante', 'Piquenique natural', 'Fotografia de natureza'],
  },
  {
    id: 5,
    ...
    history: 'Tesouro escondido da Huíla, esta cascata é conhecida pelos locais mas pouco explorada por turistas.',
  ...
  },
  {
    ...
    history: 'Fundada no período colonial, preserva e estuda a biodiversidade única do planalto angolano.',
  ...
  },
  {
    ...
    history: 'Reservatório natural que se tornou ponto de encontro para famílias e amantes da natureza.',
    ...
    duration: '1h',
    highlights: ['Piquenique familiar', 'Observação de aves', 'Relaxamento'],
  },
  {
    id: 8,
    name: 'Museu da Huíla',
    ...
    history: 'Inaugurado em 1955, preserva artefactos históricos, etnográficos e artísticos da região.',
    ...
    image: 'https://images.unsplash.com/photo-1613908853317-8606178b3e81?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhbmdvbGElMjBsYW5kc2NhcGUlMjBuYXR1cmV8ZW58MXx8fHwxNzYzMzg3Njg1fDA&ixlib=rb-4.1.0&q=80&w=1080',
    duration: '1.5h',
    highlights: ['Cultura Nhaneka-Humbe', 'Arte tradicional', 'História regional'],
  },
];

export default function CustomTour({ onBack, onNavigateToDestination }: CustomTourProps) {
  ...
  // Auto-minimize card on scroll
  useEffect(() => {
    const handleScroll = () => {
      const currentScrollY = window.scrollY;
      
      // If scrolling down past 100px, minimize the card
      if (currentScrollY > 100 && currentScrollY > lastScrollY) {
        setIsCardMinimized(true);
      }
      // If at the top, expand the card
  ...
  const calculatePrice = () => {
    ...
    return basePrice + (count - 3) * 10000;
  };
  ...
  const isFormValid = selectedCount > 0 && selectedDate !== undefined;
  ...
  const handleOpenGoogleMaps = () => {
    ...
    if (selectedDests.length > 0) {
      const waypoints = selectedDests.map(d => `${d.name}, ${d.location}, Angola`).join('|');
  ...
  return (
    <div className="min-h-full bg-neutral-50">
      ...
      <div className="bg-gradient-to-br from-teal-600 to-emerald-600 px-6 pt-12 pb-6 relative">
        ...
        <p className="text-teal-100">Monte o teu passeio personalizado pela Huíla</p>
      </div>
      ...
      <div className="px-6 py-6">
            ...
            <PopoverTrigger asChild>
              <Button
                ...
                className={`w-full justify-start text-left h-14 ${
                  !selectedDate ? 'text-neutral-400' : 'text-neutral-900'
                }`}
              >
                ...
                <div className="flex-1">
                  {selectedDate ? (
                    <div>
                      <div className="text-neutral-900">{formatDate(selectedDate)}</div>
                    </div>
                  ) : (
        ...
        <Card 
          className={`mb-6 sticky top-4 z-10 shadow-xl border-3 border-teal-600 bg-gradient-to-br from-white to-teal-50 transition-all duration-300 ${
            isCardMinimized ? 'p-4' : 'p-6'
          }`}
        >
          ...
          >
            <div className="flex items-center gap-2">
              <Car className={`text-teal-700 ${isCardMinimized ? 'w-5 h-5' : 'w-6 h-6'}`} />
              <h2 className={`text-neutral-900 ${isCardMinimized ? '' : ''}`}>
                {isCardMinimized ? 'Resumo' : 'Resumo do Passeio'}
              </h2>
            </div>
            <div className="flex items-center gap-3">
              {isCardMinimized && (
                <div className="text-right mr-2">
                  <div className="text-teal-700">{formatPrice(totalPrice)}</div>
                  <div className="text-neutral-600">{selectedCount} {selectedCount === 1 ? 'local' : 'locais'}</div>
                </div>
              )}
          ...
          {!isCardMinimized && (
            <>
              <div className="space-y-4 mt-5">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-teal-200">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-neutral-600">Locais selecionados</div>
                      <div className="text-teal-900">
                        {selectedCount} {selectedCount === 1 ? 'destino' : 'destinos'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg p-4 space-y-3 border border-teal-200">
                  <div className="flex justify-between items-center">
                    <span className="text-neutral-700">Valor base (até 3 locais)</span>
                    <span className="text-neutral-900">50.000 Kz</span>
                  </div>
                  {extraLocations > 0 && (
                    <div className="flex justify-between items-center">
                      <span className="text-neutral-700">
                        {extraLocations} {extraLocations === 1 ? 'local extra' : 'locais extras'} (+10.000 Kz cada)
                      </span>
                      <span className="text-neutral-900">{formatPrice(extraLocations * 10000)}</span>
                    </div>
                  )}
                  <div className="pt-3 border-t border-neutral-200 flex justify-between items-center">
                    <span className="text-neutral-900">Valor Total</span>
                    <div className="text-right">
                      <div className="text-teal-700">{formatPrice(totalPrice)}</div>
                      <div className="text-neutral-600">por carro</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-teal-50 border border-teal-200 rounded-lg p-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-teal-900">
                      <p className="mb-1">O passeio é cobrado por carro, independente do número de passageiros.</p>
                      <p>Preço fixo até 3 locais, depois +10.000 Kz por cada local adicional.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                {!selectedDate && selectedCount === 0 && 'Escolha data e locais'}
                {!selectedDate && selectedCount > 0 && 'Escolha uma data'}
                {selectedDate && selectedCount === 0 && 'Selecione pelo menos 1 local'}
        ...
        <div className="mb-6">
          <h2 className="text-neutral-900 mb-4">Escolhe os destinos turísticos</h2>
              ...
              return (
                <Card
                  ...
                  className={`overflow-hidden transition-all ${
                    isSelected 
                      ? 'ring-2 ring-teal-600 shadow-xl bg-teal-50 border-teal-300' 
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 py-3 pr-3 flex flex-col">
                      <div 
                        ...
                        onClick={() => toggleDestination(destination.id)}
                      >
                        <h3 className={`mb-2 ${isSelected ? 'text-teal-900' : 'text-neutral-900'}`}>
                          {destination.name}
                        </h3>
                        ...
                        <p className={`mb-3 line-clamp-2 ${isSelected ? 'text-teal-800' : 'text-neutral-600'}`}>
                          {destination.history}
                        </p>
                      </div>
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          onNavigateToDestination(destination.id);
                        }}
                      >
          ...
          <Card className="p-6 mb-24 border-2 border-teal-200">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-neutral-900">Rota do passeio</h2>
              </div>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="bg-gradient-to-br from-teal-50 to-emerald-50 rounded-lg p-6 mb-4">
              <div className="text-center mb-6">
                ...
                <h3 className="text-neutral-900 mb-2">Mapa interativo da rota</h3>
                <p className="text-neutral-600">
                  {selectedCount} {selectedCount === 1 ? 'destino selecionado' : 'destinos conectados'}
                </p>
              </div>
                    ...
                    return (
                      <div key={destId}>
                        <div className="flex items-center gap-4 p-3 bg-teal-50 rounded-lg">
                          ...
                          <div className="flex-1">
                            <p className="text-neutral-900">{dest.name}</p>
