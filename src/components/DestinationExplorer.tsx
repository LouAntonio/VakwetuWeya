...
export default function DestinationExplorer({ category, onBack }: DestinationExplorerProps) {
  const [favorites, setFavorites] = useState<number[]>([]);
  const destinations = category === 'beaches' ? beaches : nature;
  const title = category === 'beaches' ? 'Praias de Angola' : 'Natureza de Angola';
  const subtitle = category === 'beaches' ? 'Descobre as melhores praias' : 'Explora a natureza selvagem';
                ...
                <div className="relative">
                  <div className="aspect-[4/5] relative">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <h3 className="text-white mb-1">{dest.name}</h3>
                      <p className="text-neutral-200 line-clamp-2">{dest.description}</p>
                    </div>
                  </div>
                </div>
              </Card>
