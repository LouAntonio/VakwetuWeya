import { MapPin, Hotel, UtensilsCrossed, Package, Glasses, ShoppingBag, Car } from 'lucide-react';
...
interface HomeProps {
  onNavigate: (screen: Screen, packageId?: number) => void;
  onNavigateToExplorer: (category: 'beaches' | 'nature') => void;
}
...
const quickAccess = [
  { icon: Package, label: 'Pacotes', screen: 'packages' as Screen, color: 'bg-emerald-100 text-emerald-700' },
  { icon: Hotel, label: 'Hotéis', screen: 'hotels' as Screen, color: 'bg-blue-100 text-blue-700' },
  { icon: UtensilsCrossed, label: 'Restaurantes', screen: 'restaurants' as Screen, color: 'bg-orange-100 text-orange-700' },
  { icon: ShoppingBag, label: 'Artesanato', screen: 'crafts' as Screen, color: 'bg-purple-100 text-purple-700' },
  { icon: Glasses, label: 'RA 360°', screen: 'ar360' as Screen, color: 'bg-pink-100 text-pink-700' },
];

export default function Home({ onNavigate, onNavigateToExplorer }: HomeProps) {
  return (
    <div className="min-h-full bg-neutral-50">
          ...
          <div className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h2 className="text-white mb-2">Passeios pela Província</h2>
                <p className="text-teal-100 mb-4">
                  Cria o teu passeio personalizado! Escolhe os destinos e nós tratamos do resto.
                </p>
                <div className="flex items-center gap-2">
                  <Badge className="bg-white text-teal-700 hover:bg-white">A partir de 50.000 Kz</Badge>
                  <Badge className="bg-teal-700 text-white hover:bg-teal-700">Até 3 locais incluídos</Badge>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-6 mt-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button variant="ghost" onClick={() => onNavigate('packages')}>
            Ver todos
          </Button>
        </div>
            ...
            >
              <div className="relative h-48">
                ...
                <div className="absolute top-3 right-3 bg-white px-3 py-1 rounded-full">
                  <span className="text-emerald-700">{pkg.duration}</span>
                </div>
              </div>
              <div className="p-4">
                <h3 className="text-neutral-900 mb-2">{pkg.name}</h3>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-1 text-neutral-600">
                    <MapPin className="w-4 h-4" />
                    <span>{pkg.location}</span>
                  </div>
                  <span className="text-emerald-700">{pkg.price}</span>
                </div>
              </div>
            </Card>
          ))}
